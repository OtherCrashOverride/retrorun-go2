	...
	while (true)
	{
		...
		if (rc == 0)
		{
            ...
            if (ev.type == EV_KEY)
			{
                ...
                {
                    case BTN_DPAD_UP:
                        pending_state.dpad.up = state;
                        break;
                    case BTN_DPAD_DOWN:
                        pending_state.dpad.down = state;
                        break;
                    case BTN_DPAD_LEFT:
                        pending_state.dpad.left = state;
                        break;
                    case BTN_DPAD_RIGHT:
                        pending_state.dpad.right = state;
                        break;
            ...
            else if (ev.type == EV_ABS)
            {
                ...
                {
                    case ABS_X:
                        pending_state.thumb.x = ev.value / VALUE_MAX;
                        break;
                    case ABS_Y:
                        pending_state.thumb.y = ev.value / VALUE_MAX;
                        break;
                }
            }
